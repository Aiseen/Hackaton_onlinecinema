...
from apps.account.send_mail import send_confirmation_email, forgot_password_email
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if password != password2:
            raise serializers.ValidationError('Пароли разные!')
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if email and password:
            ...
            if not user:
                raise serializers.ValidationError('Ваш пароль либо почта указаны неправильно')
...
class ChangePasswordSerializer(serializers.Serializer):
    ...
    def validate_old_password(self, password):
        ...
        if not user.check_password(password):
            raise serializers.ValidationError('Пароль не тот')
        return password
